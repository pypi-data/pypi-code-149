...
from cux.auth import Accounts


def scf():
    return StrictRedis(host='cn.ddot.cc', port=13679, password=Accounts.redis_pass)


class consts(object):
    amqp = type('AMQPClass', (object, ), {
        'url': Accounts.amqp_url,
...
class JupyterSQL(object):
    ...
    def query(self) -> List[str]:
        if self.cmd is not None:
            return [e for e in self.exec(self.cmd) if e]
        return []

    def get_df(self) -> pd.DataFrame:
        import secrets
        path = f'/tmp/{secrets.token_hex(2)}.csv'
        cf.info(f'write to {path}')
        cf.io.write(self.query(), path)
...
class JupyterClientDaemon(object):
    ...
    def execute_once(self, query_str) -> pd.DataFrame:
        ...
        try:
            data = func_timeout(30, pd.read_sql_query, args=(query_str, conn))
    ...
    def callback(self, ch, method, properties, body):
        ...
        self.db.set(task_id, js, ex=1200)
