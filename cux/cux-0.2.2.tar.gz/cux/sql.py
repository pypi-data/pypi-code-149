...
class SQLClient:
    ...
    def _tunnel(self, sql_command: str):
        with SSHTunnelForwarder(
            ...
            # mysql 服务ip 和 part
            remote_bind_address=(SQL_CONFIG["sql_ip"], 3307),
            local_bind_address=(
                "127.0.0.1",
                5143,
...
class Organizations:
    ...
    def load(cls) -> dict:
        ...
        if cf.io.exists('/data/organization.csv'):
            lis = cf.io.read('/data/organization.csv')
        else:
            lis = JupyterSQL('select * from megaview_db.organization;').query()
        lis = [e for e in lis if e]
        return dict(tuple(e.split(',')[:2]) for e in lis)
...
class TranscriptionTable:
    def __init__(self, tple) -> None:
        ...
        self.company = config.APP_ORGANIZATION.get(
            str(self.organization_id), 'COMPANY UNK')
...
class AppSlave:
    ...
    def transcription(self, cid: int = 8) -> dict:
        ...
        ret = [e for e in JupyterSQL(sql).query() if e]
        if len(ret) < 2:
            cf.info(ret)
            cf.error(
                "Found no data from megaview_db.transcription table")
            return {}
