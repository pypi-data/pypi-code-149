...
from .toolkit import aconfig, alog, fileio, ObjectSerializer
from .toolkit.errors import error_handler, DataValidationError

log = alog.use_channel("MODULE")
error = error_handler.get(log)

# This file is `*` imported so we need to override what is exposed
__all__ = ["ModuleBase", "ModuleConfig", "ModuleLoader", "ModuleSaver"]
...
class ModuleBase:
    ...
    def bootstrap(cls, *args, **kwargs):
        ...
        from artifacts created outside of WatsonNLP
        """
        error(
            "<COR92634438E>",
    ...
    def _validate_and_extract_batch_size(self, *args, **kwargs):
        """Check to ensure that there's at least one iterable whose length is well defined, i.e., no
        generators, and that if multiple iterable arg/kwarg values are provided, they are all the same
        length.
    ...
    def _validate_arg_and_verify_batch_size(self, val, current_batch_size):
        ...
        if self._is_expandable_iterable(val):
            ...
            elif current_batch_size != iter_batch_size:
    ...
    def stream(self, data_stream, *args, **kwargs):
        ...
        Args:
            data_stream: watson_core.data_model.DataStream
        ...
        **kwargs
        ...
        Args:
            ...
            max_hierarchy_levels: int
                Used in hierarchical multilabel multiclass evaluation only. The number of levels in the hierarchy to run model evaluation on,
    ...
    def timed_load(cls, *args, **kwargs):
        ...
        # We are calling watson_core over cls.load because we need to figure out
        # what instance the model belongs to
        model = watson_core.load(*args, **kwargs)
    ...
    def timed_run(self, *args, num_seconds=None, num_iterations=None, **kwargs):
        ...
        Returns:
            int, int, watson_core.data_model.DataBase
...
class ModuleConfig(aconfig.Config):
    ...
    def __init__(self, config_dict):
        ...
        # validate block id, if we have one
        if self.block_id is not None:
            if isinstance(self.block_id, str):
                self.module_id = self.block_id
            else:
                error(
                    "<COR80419079E>",
                    TypeError(
                        "`{}` is invalid for `block_id`, must be a string.".format(
                            self.block_id
                        )
                    ),
                )

        # validate workflow id, if we have one
            ...
            if isinstance(self.workflow_id, str):
                ...
                error(
                    "<COR80419125E>",
                    TypeError(
                        "`{}` is invalid for `workflow_id`, must be a string.".format(
                            self.workflow_id
                        )
                    ),
                )

        # validate resource id, if we have one
            ...
            if isinstance(self.resource_id, str):
                ...
                error(
                    "<COR77356413E",
                    ValueError(
                        "`{}` is invalid for `resource_id`, must be a string.".format(
                            self.resource_id
                        )
                    ),
                )

        # error if neither block_id nor workflow_id was specified
        else:
            error(
                "<COR80418932E>",
                KeyError(
                    "Please specify `block_id` or `workflow_id` or `resource_id` "
                    "in model config."
                ),
            )
...
class ModuleSaver:
    ...
    def __init__(self, model_path, library_name, library_version):
