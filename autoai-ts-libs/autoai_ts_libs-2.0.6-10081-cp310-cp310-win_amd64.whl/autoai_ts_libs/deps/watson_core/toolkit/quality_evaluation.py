...
# ****************************************************************#
"""Evaluate quality of WatsonNLP models.
"""
...
class QualityEvaluator:
    ...
    def singlelabel_multiclass_evaluation(self, labels=None) -> dict:
        ...
        for gold_label, pred_label in zip(gold, pred):
            if gold_label not in per_class_confusion_matrix and (
                labels == None or gold_label in labels
            ):
            ...
            if pred_label not in per_class_confusion_matrix and (
                labels == None or pred_label in labels
            ):
            ...
            # true positive
            if gold_label == pred_label and (labels == None or gold_label in labels):
                per_class_confusion_matrix[gold_label].true_positive += 1
            else:
                if labels == None or pred_label in labels:
                    per_class_confusion_matrix[pred_label].false_positive += 1
                if labels == None or gold_label in labels:
    ...
    def multilabel_multiclass_evaluation(
        ...
        for gold_ex, pred_ex in zip(gold, pred):
            ...
            try:
                ...
                log.info(
                    "INFO: find_label_func not implemented for this block type - falling back to tuple match!!"
                )
                use_labels_for_matching = False
    ...
    def calc_metrics_from_confusion_matrix(
        ...
        Args:
            per_class_confusion_matrix: Dict[str, F1Metrics]
                 Dictionary of statistics per class label. Class labels are keys for the dictionary.
                 For each class label, there should be a F1Metrics class object with values
                 true positive, false_positive , false_negative representating the count of these per class.
