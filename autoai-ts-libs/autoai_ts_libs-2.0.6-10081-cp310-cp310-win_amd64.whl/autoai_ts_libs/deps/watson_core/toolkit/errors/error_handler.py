...
class ErrorHandler:
    ...
    def _handle_exception_messages(self, log_code, exception):
        ...
        if hasattr(exception, "_watson_core_nexception_log_messages"):
            exception._watson_core_nexception_log_messages += 1
        else:
            exception._watson_core_nexception_log_messages = 0

        # if less than max log messages, then omit a log message
        if exception._watson_core_nexception_log_messages < MAX_EXCEPTION_LOG_MESSAGES:
            self.log_chan.error(
        ...
        # if at the limit omit one message stating that we will no longer log
            ...
            exception._watson_core_nexception_log_messages == MAX_EXCEPTION_LOG_MESSAGES
        ):
            self.log_chan.error(
                log_code,
    ...
    def log_raise(self, log_code, exception, root_exception=None):
        ...
        Args:
            log_code:  str
                ...
                (for `watson_core` in this example) and where `12345678` is a unique eight-digit
    ...
    def type_check(self, log_code, *types, allow_none=False, **variables):
        ...
        Args:
            log_code:  str
                ...
                (for `watson_core` in this example) and where `90063501` is a unique eight-digit
                identifier (example generation in `scripts/cor_log_code`) and where `E` is an error
                level short-code, one of `{'fatal': 'F', 'error': 'E', 'warning': 'W', 'info': 'I',
                'trace': 'T', 'debug': 'D'}`.
            *types:  type or None
    ...
    def value_check(self, log_code, condition, msg="", *args):
        ...
        Args:
            log_code:  str
                ...
                (for `watson_core` in this example) and where `55705215` is a unique eight-digit
                identifier (example generation in `scripts/cor_log_code`) and where `E` is an error
                level short-code, one of `{'fatal': 'F', 'error': 'E', 'warning': 'W', 'info': 'I',
                'trace': 'T', 'debug': 'D'}`.
            condition:  bool
