...
from autoai_ts_libs.deps.watson_core.toolkit.errors import DataValidationError, error_handler
...
class DataStreamValidator:
    ...
    def __init__(self, expected_keys: Dict[str, type]):
        ...
        Args:
            expected_keys: Dict(str)
                (Ordered) dictionary of key names -> types that determines how data will be validated
        """
    ...
    def validate(self, stream: DataStream) -> DataStream:
        ...
        2. In the case of dictionaries, all expected keys exist and the values are the types given in
            `self._expected_keys`
        ...
        Returns:
            The same data stream, which will now throw DataValidationErrors when accessed if the data
            is not valid.
        """
        data_item_number = -1
    ...
    def _validate_data(self, data_item: object, data_item_number: int) -> None:
        ...
        if isinstance(data_item, dict):
        ...
        elif isinstance(data_item, list) or isinstance(data_item, tuple):
            ...
            for (type_, element, index) in zip(
                ...
                if not isinstance(element, type_):
                    message = "Expected element {} in data item to be of type {}, but got {}".format(
                        index, type_, type(element)
                    )
                    raise DataValidationError(message, data_item_number)
