...
class DataStream:
    ...
    def from_iterable(cls, data):
        ...
        Examples:
            ...
            3
        """
        error.type_check("<COR88684982E>", collections.abc.Iterable, data=data)

        # lambda closure over `data` that returns an iterator over `data`
        return cls(lambda: iter(data))

    @classmethod
    def from_json_array(cls, filename):
    ...
    @classmethod
    def from_csv(cls, filename, *args, skip=0, **kwargs):
    ...
    @classmethod
    def from_file(cls, filename: str) -> "DataStream":
        """Loads up a DataStream from a file.
            Will call the correct DataStream.from_xxx static constructor based on the file extension
        ...
        # TODO: Add CSV later after we decide how CSVs should be formatted
    ...
    @classmethod
    def _from_collection(cls, dirname, extension, file_opener):
        ...
        if not os.path.exists(dirname):
            error(
                "<COR82306771E>",
                FileNotFoundError(
                    "Could not find text collection directory `{}`".format(dirname)
                ),
            )
        ...
        if not os.path.isdir(dirname):
            error(
                "<COR82306849E>",
                NotADirectoryError(
                    "Text collection path `{}` is not a directory".format(dirname)
                ),
            )
    ...
    @classmethod
    def from_txt_collection(cls, dirname, extension="txt"):
    ...
    @classmethod
    def from_json_collection(cls, dirname, extension="json"):
    ...
    def train_test_split(self, test_split=0.25, seed=None):
        ...
        return self.__class__(train_generator_func), self.__class__(test_generator_func)
    ...
    def filter(self, func=lambda data_item: data_item, *args, **kwargs):
    ...
    def shuffle(self, buffer_size, seed=None):
        ...
        size of the full data stream, it is a partial random shuffle which is similar to Tensorflow's
        dataset shuffle. For instance, if your dataset contains 10,000 elements but buffer_size is set
        to 1,000, then shuffle will initially select a random element from only the first 1,000 elements
        in the buffer. Once an element is selected, its space in the buffer is replaced by the next
        (i.e. 1,001-st) element, maintaining the 1,000 element buffer.
        ...
        return self.__class__(generator_func)
    ...
    def eager(self):
        ...
        Returns:
            DataStream
                ...
                stream.
        """
        return self.__class__.from_iterable(list(self))
    ...
    def __getitem__(self, idx):
        ...
        Notes:
            ...
            subscriptable.
        """
        return self.__class__(lambda: (data_item[idx] for data_item in self))
