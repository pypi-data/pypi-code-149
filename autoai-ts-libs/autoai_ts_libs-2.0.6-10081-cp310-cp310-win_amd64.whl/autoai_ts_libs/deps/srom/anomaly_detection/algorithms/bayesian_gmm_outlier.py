...
"""
    .. module:: bayesian_gmm_outlier
       :synopsis: bayesian_gmm_outlier class.
...
class BayesianGMMOutlier(BayesianGaussianMixture):
    """
    Anomaly class based on Bayesian Gaussian mixture model.
    """
    ...
    def __init__(
        ...
        super(BayesianGMMOutlier, self).__init__(
    ...
    def fit(self, X, y=None):
        ...
        super(BayesianGMMOutlier, self).fit(X)
    ...
    def score_samples(self, X, call_parent=False):
        ...
        """
        if call_parent:
            return super(BayesianGMMOutlier, self).score_samples(X)
        X = check_array(X, estimator=self, dtype=FLOAT_DTYPES)
    ...
    def decision_function(self, X):
        ...
        return (self.score_samples(X) - self.likelihood_threshold_) * (-1)
