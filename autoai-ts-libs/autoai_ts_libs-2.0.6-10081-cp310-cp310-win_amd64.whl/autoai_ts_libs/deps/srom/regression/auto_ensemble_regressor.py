...
class EnsembleRegressor(BaseEstimator, RegressorMixin, SROMEnsemble):
    ...
    def fit(self, X, y):
        ...
        """
        if self.is_automate_run == False:
            self.set_params_for_auto_module(cv=self.cv, scoring=self.scoring)
            subX = X
            subY = y
            if X.shape[0] > 500:
                subX = X[:500, :]
            ...
            self.automate(subX, subY)
            self.is_automate_run = True

        if (
            self.n_leaders_for_ensemble == 1
            and self.n_estimators_for_pred_interval == 1
        ):
            ...
            if (
                isinstance(self.max_samples_for_pred_interval, int)
                and self.max_samples_for_pred_interval <= X.shape[0]
            ):
            ...
            if self.ensemble_type == "voting":
                ...
                self.auto_regression.fit_stacked_ensemble(
                    X,
                    y,
                    self.n_leaders_for_ensemble,
                    n_estimators=self.n_estimators_for_pred_interval,
    ...
    def automate(self, X, y=None):
        self.auto_regression.automate(X, y)
        return self

    def set_params_for_auto_module(self, **kwarg):
        self.auto_regression.set_params(**kwarg)
    ...
    def predict_proba(self, X):
        ...
        if (
            ...
            if self.ensemble_type == "voting":
                return self.auto_regression.predict_voting_ensemble_interval(
                    X, prediction_percentile=self.prediction_percentile
                )
            else:
                return self.auto_regression.predict_stacked_ensemble_interval(
                    X, prediction_percentile=self.prediction_percentile
                )
