...
from autoai_ts_libs.deps.srom.optimization import Optimization
...
from autoai_ts_libs.deps.srom.optimization.optimization import NonLinearOptimizer, SUPPORTED_METHODS
from autoai_ts_libs.deps.srom.wml.adapter.optimization import (
    NonLinearOptimizer as WrapperNonLinearOptimizer,
)
...
class TestOptimization(unittest.TestCase):
    ...
    def test_optimization_with_smaller_is_better(self):
        ...
        pipelines_dict = test_class.pipelines_dict
        data_for_recommendations = test_class.data_for_recommendations
        objective_function = TestOptimization.reverse_objective_function
    ...
    def test_srom_wrapper_vs_direct_call(self):

        """
        Minimize the objective function f(x1,x2) = x0*x1 subject
        to the constraints x0**2 + x1**2 < 1 and x0 > 1 and x1 > 0:
        """
        ...
        # first test by going directly to cobyla function
        x_star = fmin_cobyla(
            objective, [0.0, 0.1], [constr1, constr2, constr3], rhoend=1e-7
        )
        print("x_star", x_star)

        df = pd.DataFrame(dict(a=[0], b=[0.1], c=[0]))

        # now via the optimizer which wraps above
        for method in SUPPORTED_METHODS:
            nlopt = NonLinearOptimizer(
                estimator_dict=None,
            ...
            constraints = [
                constr1,
                constr2,
                (constr3, (1, 2, 3)),  # with args
            ]  # , constr2, (constr3, (1, 2, 3))]
            ans = nlopt.predict(df.iloc[[0]], constraints=constraints)
            tol = 1e-5

            diff = math.fabs(objective(x_star) - ans["objective_value"])
            self.assertLessEqual(diff, tol)

    def test_obj_with_varargs(self):
        ...
        class ObjectiveFunctor:
            def __call__(self, x, *args):
                # a dictionary of kwargs will
                # always be present in the last position
                # it can contain an content you like
                kwargs = args[-1]

                # change nature of objective function
                # based on what was passed in as kwargs
                # to predict
                ans = 10 if kwargs.get("linear", False) else x[0] * x[1]
                return ans
        ...
        with NonLinearOptimizer(
            ...
            # this will give us x1*x2 as the objective
            ans = nlopt.predict(df.iloc[[0]], constraints=constraints, foo="bar")
            self.assertEqual(0.0, ans["objective_value"])
            # this time around we'll change what the object does effecitively
            # by passing in linear in the kwargs
            ans = nlopt.predict(
                df.iloc[[0]], constraints=constraints, foo="baz", linear=True
            )
            self.assertEqual(10, ans["objective_value"])
