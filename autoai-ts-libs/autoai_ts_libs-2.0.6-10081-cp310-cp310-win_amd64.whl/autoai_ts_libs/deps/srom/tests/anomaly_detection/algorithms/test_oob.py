"""Unit test cases for testing oob class"""
import unittest
import numpy as np
import pandas as pd
import copy
...
class TestOOB(unittest.TestCase):
    ...
    def test_fit_method(self):
        ...
        oob_instance = OOB()
        fitted_oob = oob_instance.fit(test_class.dataframe)
        self.assertEqual(id(oob_instance), id(fitted_oob))
    ...
    def test_anomaly_score_method(self):
        ...
        oob_instance = OOB()
        fitted_oob = oob_instance.fit(test_class.dataframe)
        anomaly_score = fitted_oob.anomaly_score(test_class.dataframe)
    ...
    def test_decision_function_method(self):
        ...
        oob_instance = OOB()
        fitted_oob = oob_instance.fit(test_class.dataframe)
        prediction = fitted_oob.decision_function(test_class.dataframe)
    ...
    def test_anomaly_score_without_fit(self):
        ...
        test_class = self.__class__
        oob_instance = OOB()
        self.assertRaises(Exception, oob_instance.anomaly_score, test_class.dataframe)
