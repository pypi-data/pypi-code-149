...
class TestHBOS(unittest.TestCase):
    ...
    def test_fit_method(self):
        ...
        hbos = HBOS()
        fitted_hbos = hbos.fit(test_class.numpy_array)
        self.assertEqual(id(hbos), id(fitted_hbos))

    def test_anomaly_score_method(self):
        """Test anomaly_score method"""
        test_class = self.__class__
        hbos = HBOS()
        hbos.fit(test_class.numpy_array)
        anomaly_scores = hbos.anomaly_score(test_class.numpy_array)
        anomaly_scores = pd.DataFrame(anomaly_scores).apply(round)
        self.assertIsNotNone(hbos.decision_function(test_class.numpy_array))
