...
import numpy as np
from autoai_ts_libs.deps.srom.imputation.ts_imputer.ts_mul_var_simple_imputer import TSMulVarSimpleImputer
...
class TestMulVarSimpleImputer(unittest.TestCase):
    ...
    def test_univariate_simple_interpolation_imputer(self):
        """Tests univariate impute_simple_interpolation_imputer."""
        testclass = self.__class__
        X = testclass.integer_series
        X = X.to_numpy()
        X = np.reshape(X, (-1, 1))
        si = TSMulVarSimpleImputer()
        ...
        X = np.reshape(X, (-1, 1))
        si = TSMulVarSimpleImputer()
        result3 = si.fit_transform(X)
        self.assertTrue(result3[2] > 0.1)

    def test_multivariate_time_series_simple_imputer(self):
        """Tests multi-variate TsMulVarSimpleImputer"""
        testclass = self.__class__
        X = testclass.multivariate_time_series
        tsmulvarsimpleimputer = TSMulVarSimpleImputer()
        result = tsmulvarsimpleimputer.fit_transform(X)
