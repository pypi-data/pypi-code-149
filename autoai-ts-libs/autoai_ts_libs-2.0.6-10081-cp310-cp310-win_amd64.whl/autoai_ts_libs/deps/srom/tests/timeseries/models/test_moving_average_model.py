...
from autoai_ts_libs.deps.srom.time_series.models.moving_average import MovingAverageModel
...
class TestMovingAverageModel(unittest.TestCase):
    ...
    def test_fit(self):
        ...
        model1 = MovingAverageModel(
            target_columns=test_class.target_col, lookback_win=3
        )
    ...
    def test_predict_sliding_window(self):
        ...
        pred = model1.predict(test_class.X_test, prediction_type="forecast")
        self.assertIsNotNone(pred)
        self.assertIsInstance(pred, np.ndarray)
