...
class Encoder(nn.Module):
    ...
    def __init__(self, input_dim, num_filter, latent_dim):
        ...
        self.mu = nn.Conv1d(num_filter * 16, latent_dim, 1, 1, 0)
        self.log_var = nn.Conv1d(num_filter * 16, latent_dim, 1, 1, 0)
...
class Discriminator_Reconstruct(nn.Module):
    ...
    def __init__(self, input_dim, num_filter):
        ...
        self.classifier = nn.Sequential(
            nn.Conv1d(num_filter * 16, 1, 1, 1, 0), nn.Sigmoid()
        )
    ...
    def forward(self, x):
        ...
        """
        print('x in DCE', x.shape)
        features = self.features(x)
        print('feature size', features.shape)
        # randx = torch.rand(50,16,1)
        # conv = nn.Conv1d(1 * 16, 1, 1, 1, 0)
        # out = conv(randx)
        # print('output of conv', out.shape)
        classifier = self.classifier(features)
        print('ckassifier', classifier.shape)
...
class Discriminator_Latent(nn.Module):
    ...
    def __init__(self, num_filter):
        ...
        self.classifier = nn.Sequential(
            nn.Conv1d(num_filter * 16, 1, 4, 1, 4),
            nn.Sigmoid()
        )
...
class Generator(nn.Module):
    ...
    def forward(self, x):
        ...
        print('latentz shape', latent_z.shape)
        output = self.decoder(latent_z)
        return output, latent_z, mu, log_var


class DAEMON(BaseAutoEncoderAnomalyDetector):
    def __init__(
        self,
        input_dim,
        num_filter,
    ...
    def train_epoch_(self, X):
        ...
        for data in X:
            ...
            print('data size in batch', data.shape)
            ...
            self.update_d_lat_(train_batchsize, p_z, batch_x)
    ...
    def update_d_rec_(self, train_batchsize, batch_x):
        ...
        self.real_label = 1.0
        self.fake_label = 0.0
        self.out_d_rec_real, self.feat_rec_real = self.D_rec_(batch_x)

        self.out_g_fake, self.latent_z, _, _ = self.G_(batch_x)
        print('out_g_fake', self.out_g_fake.shape)
        self.out_d_rec_fake, self.feat_rec_fake = self.D_rec_(self.out_g_fake.detach())
        print('out_d_rec_fake', self.out_d_rec_fake.shape, self.feat_rec_fake.shape)

        self.loss_d_rec_real = self.bce_criterion_(self.out_d_rec_real,
                                                  torch.full((train_batchsize,), self.real_label,
                                                             device=self.device))
        self.loss_d_rec_fake = self.bce_criterion_(self.out_d_rec_real,
                                                  torch.full((self.train_batchsize,), self.fake_label,
                                                             device=self.device))
        ...
        # print('first call', batch_x.shape)
        # out_d_rec_real, _ = self.D_rec_(batch_x)
        # out_g_fake, _, _, _ = self.G_(batch_x)
        # print('second call', out_g_fake.detach().shape)
        ...
        # )
        # print('input', out_d_rec_fake.shape, 'target', train_batchsize)
    ...
    def update_d_lat_(self, train_batchsize, p_z, batch_x):
        ...
        loss_d_lat_real = self.bce_criterion_(
            out_d_lat_real,
            torch.full((train_batchsize,), real_label, device=self.device),
        )
        loss_d_lat_fake = self.bce_criterion_(
            out_d_lat_fake,
            torch.full((train_batchsize,), fake_label, device=self.device),
        )
