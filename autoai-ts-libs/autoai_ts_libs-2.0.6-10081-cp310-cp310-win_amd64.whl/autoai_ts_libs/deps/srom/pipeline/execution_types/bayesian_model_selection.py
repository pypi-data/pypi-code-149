...
"""
.. module:: bayesian_model_selection
   :synopsis: This module use bayesian grid search.
...
# temporary fix for testing - sklearn version 1.0.2
...
def bayesian_search(
    ...
    Parameters:
        ...
        param_grid (dict): Dictionary with parameter names(string) as keys and lists \
            of parameter settings to try as values, or a list of such dictionaries, in \
        ...
        pre_dispatch (integer, or string): Controls the number of jobs that get dispatched \
        ...
        max_eval_time_minute (integer) (minutes): Maximum timeout for execution of pipelines \
            with unique parameter grid combination.
        ...
        base_estimator_name (string): "GP", "RF", "ET", "GBRT" or sklearn regressor, default="GP".
    ...
    Raises:
        IncorrectValueException:
            1. If base_estimator_name not in ['RF', 'GP', 'ET', 'GBRT'].
    ...
    """
    # Validations
    if base_estimator_name not in ["RF", "GP", "ET", "GBRT"]:
        raise IncorrectValueException(
            "Supported mode should be provided: 'RF', 'GP', 'ET', 'GBRT'"
        )
    if not paths:
    ...
    for path in paths:
        ...
        try:
            ...
            evaluator = BayesSearchCV(
                ...
                optimizer_kwargs={"base_estimator": base_estimator_name},
                verbose=verbose,
            )
