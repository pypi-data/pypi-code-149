...
##THIS Class is not up to date with pointwise_bounded_ad and should be depreicated in some time.
class PointwiseBoundedForecastingDetector(TSADEstimator):
    ...
    tspy.forecasters.anomaly_detector which does not update the underlying forecasting model. The anomaly_detector
    ...
    def fit(self, X, y=None, _reset_model=True, **fit_params):
        ...
        # model = tspy.forecasters.bats(training_sample_size)
        # confidence = 0.90
        # ad = tspy.forecasters.anomaly_detector(confidence)
    ...
    def anomaly_score(self, X=None, **predict_params):
        ...
        # TODO : get the bounds at for each model and for each value in X
        # TODO: Need to check what is the time stamp and compute future time stamps for getting the bounds
        # TODO: X cannot be none as we are scoring for anomaly and X should not be passed down to the model otherwise
    ...
    #probably not needed as pickle is now supported downstream in tspy
