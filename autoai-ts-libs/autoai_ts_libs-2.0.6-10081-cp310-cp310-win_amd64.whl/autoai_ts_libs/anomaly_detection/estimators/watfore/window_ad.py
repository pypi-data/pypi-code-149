...
class WindowedKNNAnomalyDetector(TSADEstimator):
...
#        self.all_args['algorithm'] = self.algorithm
    ...
    def fit(self, X, y=None, _reset_model=True, **fit_params):
        ...
        #-- Generate AD model, each for ONE variable/component time series:
        ...
        if self.debug:
            print("==============MODELS Created==============",self.model) #-- error: no model for WindowedKNNAnomalyDetector
        ...
        for col in range(0, self.n_targets_):
            try:
                ...
                for i in range(0,len(vals_df_array[col])):
                    # if (i%5000==0):
                    #     print(f"** (For Debugging) train_ad_model: col {col} i {i}")
