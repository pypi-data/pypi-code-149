...
class PointwiseBoundedAnomalyDetector(TSADEstimator):
    ...
    to the tspy.forecasters.updatable_anomaly_detector to create anomaly detector object.The anomaly detector objects
            ...
            for col in range(0, self.n_targets_):
                ...
                for row in range(0, X.shape[0]):
                    try:
                        ...
                        ad = tspy.forecasters.updatable_anomaly_detector(
                            model=model_cp[col],
                            confidence=self.confidence_interval,
                            update_anomalies=self.update_with_anomaly,
                        )
    ...
    def predict(self, X=None):
        ...
        # print('TS provided',self.ts_icol_loc)
        try:
            # check_is_fitted(self, 'is_fitted_', msg=Messages.get_message(message_id='AUTOAITSLIBS0056E'))
            check_is_fitted(self, "is_fitted_", msg="Model not fitted.")
        except BaseException as e:
            ...
            for row in range(0, X.shape[0]): #-- loop through each row in X to identify its Anomaly label by each model m (per col in target)
                ...
                for col in range(0, self.n_targets_):
                    try:
                        ...
                        # print('Col == ', col,' Last updated=',model_cp[col].last_time_updated,' is initialized==',model_cp[col].is_initialized() )
                        # print('last timestamp ==',WatForeADUtils.get_last_updated(self),' MODEL CP',WatForeADUtils.get_last_updated(model_cp))
                        print(e)
                all_bounds.append(per_model_bds)
                # for ts in pred_timestamps:
                #     print('Predict TS=', type(ts[-1]))
