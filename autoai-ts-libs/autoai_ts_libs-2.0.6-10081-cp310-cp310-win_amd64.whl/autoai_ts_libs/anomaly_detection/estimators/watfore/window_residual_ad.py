...
from autoai_ts_libs.anomaly_detection.estimators.utils.wf_ad_utils import WatForeADUtils
#from autoai_ts_libs.deps import tspy
...
class WindowedResidualAnomalyDetector(TSADEstimator):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.ts_icol_loc = (
            []
        )  # ts_icol_loc # this override is to not provide timestamps to windowed one
        ...
        self.anomalies_replaced = (
            False  # will be set if anomalous datapoints in training data are replaced
        )
    ...
    def get_pipeline_name(self):
        ...
        TODO: The order among these properties might need revision to ensure consistency
        and meeting expectation from Dev side.
        """
        pipeline_name =  "WindowResidual" + self.algorithm.upper()
        return pipeline_name

    def fit(self, X, y=None, _reset_model=True, **fit_params):

        # Parse data into format that tspy understnad
        ...
        if self.model is None:
            ...
            self.model = (
                []
            )  # self._getModel(algorithm=self.algorithm, kw_args=self.all_args)
            self.anomaly_detector = []
        ...
        if self.ts_icol_loc == -1 or len(self.ts_icol_loc) == 0:
            ...
            for t in ts:
                self._timestamps.append(
                    WatForeADUtils.get_unixtimestamp(t)
                )  # Assuming LONG format timestamp
            # [X.flatten().tolist()]
        ...
        if (
            ...
            or self.history_length < self.k_nearest_windows + self.window_size
        ):
            self.history_length = len(vals_df_array[0])
        ##########################CREATE AD##########################
        for m in self.model:
            # create window anomaly detector
            w_ad = autoai_ts_libs.deps.tspy.forecasters.windowed_knn_anomaly_detector(
                self.confidence_interval,
                self.k_nearest_windows,
                self.min_windows,
            ...
            adm = autoai_ts_libs.deps.tspy.forecasters.windowed_residual_anomaly_detector(
                regular_interval=self.regular_interval,
                ...
                # IF NOT ARIMA just Train model
                    ...
                    if self.algorithm.lower() in EXOGENOUS_MODELS:
                        ...
                        for i in range(0, len(vals_df_array[col])):
                            self.anomaly_detector[col].is_anomaly(
    ...
    def anomaly_score(self, X=None, **predict_params):
        raise NotImplementedError("Anomaly score method not yet implemented.")
            ...
            if self.ts_icol_loc != -1 and len(self.ts_icol_loc) != 0:
                ...
                for m_col in range(0, self.n_targets_):
                    ...
                    for t in range(0, X.shape[0]):
                        prev_ts = (
                            prev_ts + self.model[m_col].average_interval
                        )  # assuming model is initialized and non-exogenous
                        ...
                        per_model_bds.append(
                            WatForeADUtils.get_anomaly_label(
                                model_copies[col].is_anomaly(
                                    ...
                                    else int(multi_col_pred_timestamps[col][row]),
                                    actual_value,
                                )
                            )
                        )
    ...
    # Remove these functions one support is added to TSPY
    ...
    #             adm = tspy.forecasters.updatable_anomaly_detector(model=m, confidence=self.confidence_interval,
