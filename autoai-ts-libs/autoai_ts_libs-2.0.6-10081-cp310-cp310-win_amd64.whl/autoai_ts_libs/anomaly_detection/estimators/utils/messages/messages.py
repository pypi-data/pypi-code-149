...
import autoai_ts_libs.anomaly_detection.estimators.utils.messages


def get_message_dict(locale):
    file_name = "messages_" + locale + ".json"
    message_dict = {}
    path = os.path.dirname(autoai_ts_libs.anomaly_detection.estimators.utils.messages.__file__)
    messages = []
    ...
    # load the english dictionary if the json file for the locale doesn't exist
        ...
        try:
            ...
            raise Exception(
                ...
                "Please ensure that the json file exists and is placed in the appropriate folder".format(
                    locale
                )
            )
