...
def run(input: Spec, output_spec: Spec):
    ...
    # return coerce_string_into_spec(output_spec, result)
...
def coerce_string_into_spec(output_spec, result):
    parsed = parse(output_spec.__doc__)
    ...
    for param in parsed.params:
        prompt = result + "\n\n"
        prompt += "Answer each question followed by a ! For example.  What is two plus two? Four ! What was the " + param.description + "?"
        output_values[param.arg_name] = PowerML().predict(prompt)
